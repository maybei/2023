...
def parse_args(argv=None):
    ...
    parser.add_argument('--trained_model',
                        default='./weights/yolact_plus_resnet50_slab_52_2000_DS.pth', type=str,
    ...
    parser.add_argument('--config', default='yolact_resnet50_slab_config',
    ...
    #parser.add_argument('--images', default='./data/slab/real_test:output_images', type=str,
    #parser.add_argument('--images', default=edit_path, type=str,
    parser.add_argument('--images', default='./data/slab/real_test?output_2', type=str,
    ...
    if args.display_text or args.display_bboxes:
        for j in reversed(range(num_dets_to_consider)):
            ...
            if args.display_text:
                ...
                #save result
                file = open(resultTextFileName, "a");
                print('coordinate [ ' + str(g_x1) + ' , ' + str(g_x2) + ' , ' + str(g_y1) + ' , ' + str(g_y2) + ' ]')
                file.write(imageName +  "[" + str(score) +'] , [ ' + str(g_x1) + ' , ' + str(g_x2) + ' , ' + str(g_y1) + ' , ' + str(g_y2) + " ] " +  "\n")
                file.close()
    return img_numpy
...
def evalimage_KDG(net: Yolact, path: str, save_path: str = None):
    img = cv2.imread(path)
    frame = torch.from_numpy(cv2.imread(path)).cuda().float()
    batch = FastBaseTransform()(frame.unsqueeze(0))
    preds = net(batch)

    frame_white = torch.from_numpy(cv2.imread("D:\WHITE_IMAGE_1600_800.jpg")).cuda().float()


    img_numpy = prep_display(preds, frame_white, None, None, undo_transform=False)
...
def evalimages_KDG(net: Yolact, input_folder: str, output_folder: str):
    ...
    while True:
        ...
        for p in Path(input_folder).glob('*'):
            ...
            evalimage_KDG(net, path, out_path)
            print(path + ' -> ' + out_path)
            # print( 'coordinate [ ' + str(g_x1) +' , ' + str(g_x2)+' , '  + str(g_y1)+' , '  + str(g_y2) + ' ]')
...
def evaluate_KDG(net: Yolact, dataset, train_mode=False):
    ...
    if args.images is not None:
        ...
        ## set result save
        global resultTextFileName
        resultTextFileName = out + '/result.txt'

        evalimages_KDG(net, inp, out)
